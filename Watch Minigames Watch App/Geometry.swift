...
//  Minigames Watch App
//
//  Vector math, spline sampling and fairway boundary construction.
//
...
// MARK: - Fairway geometry/// The playable surface: one or more smooth centerlines, each swept by a
...
struct FairwayGeometry {
    ...
    init(waypoints: [Waypoint]) { self.init(paths: [waypoints]) }
    ...
    private static func samplePath(_ waypoints: [Waypoint]) -> ([Vec2], [Double])? {
        ...
        // Drop nearly-duplicate consecutive samples.
